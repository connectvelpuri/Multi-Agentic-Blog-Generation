...
# Local storage directory fallback (when R2 not configured)
...
def _r2_is_configured() -> bool:
    return bool(
        app_settings.r2.ACCOUNT_ID
        and app_settings.r2.ACCESS_KEY_ID
        and app_settings.r2.SECRET_ACCESS_KEY
    )
...
class R2StorageService:
    ...
    def __init__(self):
        raw_bucket = app_settings.r2.BUCKET_NAME or ""
        ...
        if self._r2_enabled:
            self.client = boto3.client(
                service_name="s3",
                endpoint_url=(
                    f"https://{app_settings.r2.ACCOUNT_ID}"
    ...
    def put_object(
        self, key: str, data: str, content_type: str = "text/plain"
    ) -> bool:
        if self._r2_enabled:
            try:
    ...
    def save_articles_json(
        self, domain: str, articles: List[Dict[str, Any]]
    ) -> bool:
